...
fn doc_con_una_entidad() -> (Document, CommandBus, EntityId) {
    ...
    bus.apply(&mut doc, spawn(1, "pieza"))
        .expect("spawn inicial");
    (doc, bus, e(1))
}
...
fn spawn_crea_la_entidad_y_devuelve_su_id() {
    ...
    assert_ne!(
        s.fingerprint(),
        vacio,
        "la huella no cambió tras crear una entidad"
    );
}
...
fn cada_comando_de_escena_produce_su_cambio() {
    ...
    bus.apply(&mut doc, Command::ClearGeometry { entity: x })
        .unwrap();
    bus.apply(
        &mut doc,
        Command::SetParent {
    ...
    assert_eq!(
        s.get::<Geometry>(x),
        None,
        "ClearGeometry no quitó el componente"
    );
...
fn undo_y_redo_recorren_el_historial() {
    ...
    assert_eq!(
        ...
        antes,
        "el undo no restauró el estado exacto"
    );
    ...
    assert_eq!(
        doc.snapshot().fingerprint(),
        despues,
        "el redo no restauró el estado exacto"
    );
}
...
fn todas_las_variantes() -> Vec<Command> {
    vec![
        ...
        Command::SetGeometry {
            entity: e(1),
            payload: GeometryPayload::Brep(blob("b")),
        },
...
fn cada_variante_sobrevive_a_la_ida_y_vuelta_cbor() {
    ...
    assert_eq!(
        vistas.len(),
        12,
        "faltan variantes en la muestra: {vistas:?}"
    );
}
...
fn guion() -> Vec<Command> {
    vec![
        ...
        Command::SetGeometry {
            entity: e(11),
            payload: GeometryPayload::Mesh(blob("m")),
        },
        Command::SetVisible {
            entity: e(12),
            visible: false,
...
fn el_log_reproducido_da_la_misma_huella() {
    ...
    let mut bus = CommandBus::new();
    for c in guion() {
        bus.apply(&mut original, c)
            .expect("el guion debe aplicarse limpio");
    }
...
fn un_grupo_de_tres_comandos_se_deshace_con_un_solo_undo() {
    ...
    for n in 1..=3u128 {
        ...
        assert_eq!(
            out.entity(),
            Some(e(n)),
            "el id debe estar resuelto ya al encolar"
        );
    }
    assert_eq!(
        doc.snapshot().fingerprint(),
        inicial,
        "el grupo se aplicó antes de cerrarse"
    );
    ...
    assert_eq!(
        doc.snapshot().fingerprint(),
        inicial,
        "un undo no revirtió el grupo entero"
    );
...
fn los_mismos_tres_comandos_sin_agrupar_necesitan_tres_undos() {
    ...
    assert_ne!(
        doc.snapshot().fingerprint(),
        inicial,
        "un solo undo no debería bastar"
    );
    bus.apply(&mut doc, Command::Undo).unwrap();
    assert_ne!(
        doc.snapshot().fingerprint(),
        inicial,
        "dos undos tampoco deberían bastar"
    );
    bus.apply(&mut doc, Command::Undo).unwrap();
    assert_eq!(
        doc.snapshot().fingerprint(),
        inicial,
        "el tercer undo sí debía dejarlo vacío"
    );

    // y el estado agrupado equivalente es el mismo estado
    let mut doc2 = Document::new();
    let mut bus2 = CommandBus::new();
    bus2.apply(&mut doc2, Command::BeginGroup { label: "g".into() })
        .unwrap();
    for n in 1..=3u128 {
...
fn un_comando_sobre_una_entidad_inexistente_falla_y_no_toca_el_documento() {
    ...
    for cmd in [
        ...
        assert!(
            err.to_string().contains("crea la entidad"),
            "{kind}: el error no dice qué hacer"
        );
    }
    ...
    assert_eq!(
        doc.snapshot().fingerprint(),
        antes,
        "un comando fallido dejó rastro"
    );
...
fn un_grupo_sin_cerrar_al_final_es_un_error_y_no_aplica_nada() {
    ...
    assert_eq!(
        doc.snapshot().fingerprint(),
        inicial,
        "un grupo sin cerrar aplicó algo"
    );
...
fn un_log_con_un_grupo_sin_cerrar_no_se_reproduce() {
    ...
    assert_eq!(
        log.replay_into_new().unwrap_err(),
...
fn undo_y_redo_no_caben_dentro_de_un_grupo() {
    ...
    assert_eq!(
        bus.apply(&mut doc, Command::Undo).unwrap_err(),
        CommandError::NoAgrupable("Undo")
    );
    assert_eq!(
        bus.apply(&mut doc, Command::Redo).unwrap_err(),
        CommandError::NoAgrupable("Redo")
    );
...
fn deshacer_en_el_vacio_es_un_error_tipado() {
    ...
    assert_eq!(
        bus.apply(&mut doc, Command::Undo).unwrap_err(),
        CommandError::NadaQueDeshacer
    );
    assert_eq!(
        bus.apply(&mut doc, Command::Redo).unwrap_err(),
        CommandError::NadaQueRehacer
    );
...
fn spawn_no_pisa_una_entidad_viva() {
    ...
    assert_eq!(err, CommandError::EntidadDuplicada(e(1)));
    assert_eq!(
        doc.snapshot().get::<Name>(e(1)),
        Some(&Name("pieza".into()))
    );
}
...
fn emparentar_en_ciclo_se_rechaza() {
    ...
    bus.apply_all(
        &mut doc,
        [spawn(1, "abuelo"), spawn(2, "padre"), spawn(3, "hijo")],
    )
    .unwrap();
    bus.apply(
        &mut doc,
