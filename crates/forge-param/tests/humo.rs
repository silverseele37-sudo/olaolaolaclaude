...
fn arbol_simple_evalua_y_produce_forma() {
    let k = kernel();
    let tree = arbol_extrude_poligono(fid(1), fid(2), 6, 10.0, Plano::default(), forge_math::DVec3::Z, 5.0);
...
fn fillet_capturado_se_revincula_tras_un_cambio_de_distancia() {
    let k = kernel();
    let mut tree = arbol_extrude_poligono(fid(1), fid(2), 6, 10.0, Plano::default(), forge_math::DVec3::Z, 5.0);
    ...
    assert!(r.exacta(), "se esperaba genealogia exacta, salio {:?}", r.binding);
