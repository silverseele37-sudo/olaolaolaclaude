...
//! contrato de `forge-render-api` lo exige) y la que hace que el editor y el
//! reproductor sin editor recorran **el mismo camino**: cualquiera que sepa
//! producir un `Snapshot` ya sabe alimentar cualquier `Renderer`.
//!
//! Vivía dentro de `forge-ui`, que es donde se escribió y donde su propia
//! documentación decía que «un futuro `forge-runtime` comparta exactamente este
//! mismo camino». Cuando ese runtime llegó no podía: depender de `forge-ui`
//! sería justo lo que un reproductor sin editor no puede hacer. Copiar la
//! función habría sido peor —dos versiones de la regla de qué se dibuja, que se
//! separan a la primera— así que se sacó a su propia caja. `forge-ui` la
//! reexporta y su API pública no cambió.
//!
//! # El hash de una instancia es el del **documento**
//!
//! `DrawInstance::mesh` lleva el hash del blob tal y como aparece en el
//! documento, no el del contenido ya decodificado. Quien implemente el
//! `MeshProvider` del renderer tiene que indexar por ese mismo hash. Es una
//! trampa real y silenciosa: `forge_render_cpu::MapaDeMallas::insertar` indexa
//! por el hash de la malla decodificada, que **no** es el mismo número, y una
//! escena montada con esa pareja no dibuja nada sin dar ningún error, porque
//! una malla que no se encuentra «no es un error» y el frame sigue. Para el
//! camino del documento existe `insertar_con_hash`.
//!
