...
fn to_mesh_conserva_la_identidad_de_cada_cara() {
    ...
    assert_eq!(
        m.vertex_count(),
        8,
        "no se soldaron los duplicados del teselado"
    );
    ...
    assert_eq!(
        m.prov.cobertura(),
        1.0,
        "alguna cara perdio procedencia al cruzar"
    );
    for i in 0..6u64 {
        assert_eq!(
            m.prov.caras_de(cara(i)).len(),
            2,
            "la cara {i} deberia dar 2 triangulos"
        );
    }

    assert!((m.area() - 600.0).abs() < 1e-9, "area {}", m.area());
    assert!(
        (m.signed_volume() - 1000.0).abs() < 1e-9,
        "volumen {}",
        m.signed_volume()
    );
}
...
fn to_mesh_rechaza_teselados_invalidos() {
    ...
    assert!(
        to_mesh(&t).is_err(),
        "procedencia incompleta deberia rechazarse"
    );
...
fn subdivision_de_un_cubo_da_los_recuentos_exactos() {
    let c = cubo(2.0, f());
    assert_eq!(
        (c.vertex_count(), c.edge_count(), c.face_count()),
        (8, 12, 6)
    );
    assert_eq!(c.euler(), 2);
    ...
    n1.validate().unwrap();
    assert_eq!(
        (n1.vertex_count(), n1.edge_count(), n1.face_count()),
        (26, 48, 24)
    );
    ...
    n2.validate().unwrap();
    assert_eq!(
        (n2.vertex_count(), n2.edge_count(), n2.face_count()),
        (98, 192, 96)
    );
...
fn la_subdivision_produce_n_cuadrilateros_por_cada_n_gono() {
    ...
    assert_eq!(
        n1.face_count(),
        12,
        "4 triangulos x 3 = 12 cuadrilateros, no 16"
    );
    assert_eq!(n1.euler(), 2);
...
fn la_subdivision_acota_los_niveles() {
    let c = cubo(1.0, f());
    assert!(
        Subdivide::new(7).apply(&c).is_err(),
        "7 niveles son 4^7 caras: deberia negarse"
    );
}
...
fn espejo_duplica_las_caras_y_el_volumen() {
    ...
    let m = Mirror {
        punto: DVec3::new(3.0, 0.0, 0.0),
        normal: DVec3::X,
        soldar_costura: false,
    }
    .apply(&c)
    .unwrap();
    m.validate().unwrap();
    ...
    assert!(
        (m.signed_volume() - 2.0 * v0).abs() < 1e-9,
        "volumen {}",
        m.signed_volume()
    );
}
...
fn array_multiplica_caras_y_volumen_por_n() {
    ...
    for n in [1u32, 2, 5] {
        let a = Array {
            copias: n,
            desplazamiento: DVec3::new(10.0, 0.0, 0.0),
        }
        .apply(&c)
        .unwrap();
        a.validate().unwrap();
...
fn triangular_convierte_cada_cuadrilatero_en_dos() {
    ...
    assert!(
        (t.signed_volume() - c.signed_volume()).abs() < 1e-9,
        "el volumen no debe cambiar"
    );
...
fn soldar_colapsa_duplicados_y_descarta_caras_degeneradas() {
    ...
    assert_eq!(
        w.face_count(),
        12,
        "las caras siguen siendo 12, ahora compartiendo vertices"
    );
    assert!(
        Weld { epsilon_mm: 0.0 }.apply(&c).is_err(),
        "epsilon nulo deberia rechazarse"
    );
}
...
fn la_pila_completa_conserva_toda_la_procedencia() {
    ...
    let pila = ModifierStack::new()
        ...
        .push(Mirror {
            punto: DVec3::new(20.0, 0.0, 0.0),
            normal: DVec3::X,
            soldar_costura: false,
        })
        .push(Array {
            copias: 3,
            desplazamiento: DVec3::new(0.0, 50.0, 0.0),
        })
        .push(Triangulate);
    ...
    for i in 0..6u64 {
        assert!(
            !salida.prov.caras_de(cara(i)).is_empty(),
            "se perdio la cara {i}"
        );
    }
}
...
fn la_pila_tiene_clave_de_cache_estable() {
    let a = ModifierStack::new()
        .push(Subdivide::new(2))
        .push(Triangulate);
    let b = ModifierStack::new()
        .push(Subdivide::new(2))
        .push(Triangulate);
    let c = ModifierStack::new()
        .push(Subdivide::new(3))
        .push(Triangulate);
    let d = ModifierStack::new()
        .push(Triangulate)
        .push(Subdivide::new(2));
    ...
    assert_ne!(
        a.params_hash(),
        c.params_hash(),
        "cambiar un parametro debe cambiar la clave"
    );
    assert_ne!(
        a.params_hash(),
        d.params_hash(),
        "el orden de la pila es semantica"
    );
}
...
fn una_referencia_a_algo_que_ya_no_existe_sale_rota() {
    ...
    assert_eq!(r.len(), 1);
    assert!(
        r[0].binding.is_broken(),
        "deberia estar Broken, salio {:?}",
        r[0].binding
    );
...
fn validate_detecta_las_formas_de_estar_rota() {
    ...
    assert!(
        sin_prov.validate().is_err(),
        "no detecto mapa de procedencia descuadrado"
    );
    ...
    assert!(
        repetido.validate().is_err(),
        "no detecto vertice repetido en un bucle"
    );
