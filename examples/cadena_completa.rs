...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   volumen {:.1} mm³   area {:.1} mm²",
        props.volume_mm3, props.area_mm2
    );
    ...
    let con_chaflan = k.chamfer(
        solido,
        &[arista],
        ChamferSpec::Symmetric { distance_mm: 3.0 },
        feat,
    )?;
    ...
    println!(
        "   quitó {:.1} mm³ = (3²/2)·{:.0}, o sea una arista de {:.0} mm",
        props.volume_mm3 - v2,
        largo,
        largo
    );
    ...
    tess.validate()?;
    println!(
        "   teselado: {} triángulos, {} aristas clasificadas",
        tess.triangle_count(),
        tess.edges.len()
    );
    println!(
        "   se dibujan {} (el resto son tangentes o costuras)",
        tess.edges.iter().filter(|e| e.kind.se_dibuja()).count()
    );

    let malla = forge_mesh::to_mesh(&tess)?;
    println!(
        "   malla: {} vértices, {} caras, Euler = {}",
        malla.vertex_count(),
        malla.face_count(),
        malla.euler()
    );
    println!(
        "   procedencia conservada: {:.0}%",
        malla.prov.cobertura() * 100.0
    );
    println!(
        "   la cara seleccionada son ahora {} triángulos",
    ...
    println!(
        "   procedencia conservada: {:.0}%",
        final_.prov.cobertura() * 100.0
    );

    let sobreviven = final_.prov.caras_de(seleccionada).len();
    println!(
        "\n   \x1b[1mla cara seleccionada aguas arriba sigue localizable: {} caras\x1b[0m",
        sobreviven
    );
    ...
    println!(
        "   extremo superior en glTF = {:.4} m  (era {:.1} mm en Z)",
        max[1].as_f64().unwrap(),
        final_
            .positions
            .iter()
    ...
    println!(
        "   soporte.obj   {} bytes (Z-arriba nativo)",
    ...
    println!(
        "   documento: {} entidades, {} blobs referenciados",
        snap.entity_count(),
    ...
    println!(
        "   recargado en un almacén vacío: huella {}",
        if recargado.snapshot().fingerprint() == snap.fingerprint() {
            "IDÉNTICA"
        } else {
            "DISTINTA"
        }
    ...
    store.import_bytes(
        "/piezas/soporte.forge",
        &std::fs::read(&ruta)?,
        AssetMeta::new("Soporte en L (documento)", AssetType::Documento).with_tags(["mecanica"]),
    )?;

    let encontrados = store.search(&AssetQuery::new().with_any_tags(["mecanica"]))?;
    println!(
        "   biblioteca: {} activos, {} con la etiqueta «mecanica»",
        store.len()?,
        encontrados.len()
    );
